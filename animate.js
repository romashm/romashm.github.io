...
let canvas2 = document.getElementById('bg-canvas2'); 
...
function drawShape(x, y, r, angle, color, shape, fill) {
    ...
    if (shape === 0) {
        ...
        ctx2.moveTo(x + r * Math.cos(angle), y + r * Math.sin(angle));
        for (let i = 0; i < 2 + shape; i++) {
            angle += 2 * PI / (2 + shape);
            ctx2.lineTo(x + r * Math.cos(angle), y + r * Math.sin(angle))
        }
    }
    fill ? ctx2.fill() : ctx2.stroke()
